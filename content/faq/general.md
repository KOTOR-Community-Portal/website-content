...
KotOR is quite an old game, though, and therefore may sometimes have trouble running on modern machines even if your system meets the raw hardware specifications. In particular, integrated CPU graphics and Arc graphics have been known to cause significant issues, as well as trying to run the game on certain laptops. About 5-10% of laptops tend to be unable to run the game no matter what is attempted.

As a note, the games behave *less stably* when in compatibility mode. Aside from swoop races (basically like pod racing) in KOTOR 2, compatibility mode for older operating systems should be left off at all times.

If you have problems with *crashes specifically*, the four most common culprits are:
...
Although this is a simplification, there are two basic ways mods are installed: as *loose-file mods* and *installer mods*.

* Loose-file mods, so-called because the mod files are most typically installed by just dragging and dropping them, are the most basic, and therefore most common, mod type. By creating a folder called "override" in your main game directory, files from mods can be placed directly into the folder, which will instruct the game to utilize these modified files instead of the basegame files of the same name. This has implications for compatibility, however, since any files of the same name included in other mods will force you to choose one over the other, perhaps with game-breaking consequences. Installing mods in this manner can also rapidly grow out-of-hand—it's difficult to track what files came from where when you have 4,000 in your override folder.
